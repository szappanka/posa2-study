...
A **komponens** ebben a kontextusban egy önálló, cserélhető szoftvermodul, amely egy adott szolgáltatást nyújt, és egy egységes interfészen keresztül vezérelhető.
...
A hagyományos megközelítés: leállítod az alkalmazást, átírod, újrafordítod, újraindítod. Egy éjjel-nappal futó szerveren ez elfogadhatatlan, mert minden leállás kiesést jelent.
...
Ez a forma a **DLL** (dynamically linked library, dinamikusan linkelt könyvtár). A DLL egy olyan fájl, amelyet az operációs rendszer futás közben tud betölteni a program memóriájába, és ugyanúgy el is tud távolítani belőle anélkül, hogy a program többi részét érintené.
...
**Component (Komponens interfész)**: egységes interfészt definiál a konfigurációra és vezérlésre. Minden konkrét komponensnek tudnia kell inicializálódni, futni, szünetelni, folytatni és leállni.
...
**Concrete Component (Konkrét komponens)**: a Component interfész konkrét implementációja, DLL fájlba csomagolva. Az alkalmazás ezeket tölti be és távolítja el futás közben.

**Component Repository (Komponenstár)**: nyilvántartja az éppen betöltött és aktív komponenseket. Az edző névsorának felel meg.
...
**Component Configurator (Komponensbeállító)**: elvégzi a tényleges munkát. Értelmez egy konfigurációs szkriptet, betölti a megfelelő DLL-t, meghívja a komponens `init()` metódusát, bejegyzi a Repository-ba, és szükség esetén eltávolítja. Egyetlen példányban létezik az alkalmazásban (Singleton façade).
...
**Komponens elindítás**: a Component Configurator dinamikusan csatol egy komponenst az alkalmazáshoz és inicializálja azt. A sikeres inicializáció után a komponenst hozzáadja a Component Repository-hoz.

**Komponens használat**: az alkalmazáshoz való csatlakozás után a komponens különböző feladatokat végezhet, például más komponensekkel való üzenetváltás vagy kérések kiszolgálása. A Configurator ideiglenesen felfüggesztheti és újraindíthatja a komponenst, például a többi komponens átkonfigurálása esetén.

**Komponens leállítás**: a Configurator leállítja azokat a komponenseket, amelyekre nincsen szükség. Leállítás során a komponenseknek fel kell szabadítaniuk az erőforrásaikat. A sikeres leállás után a Configurator eltávolítja a komponenst a Repository-ból és az alkalmazás címteréből.
...
Minden komponensnek támogatnia kell az `init()`, `fini()`, `suspend()`, `resume()` és `info()` műveleteket. Az interfész megvalósítható öröklődés alapon (közös absztrakt osztály) vagy üzenetalapú stratégiával.
...
A Configurator Singleton Facade-ként valósul meg. Értelmez egy szkriptfájlt, amely megmondja, melyik DLL-ből melyik factory függvényt kell meghívni. A DLL-betöltő OS API-kat Wrapper Facade rejti el:
...
- **In-band**: szinkron módon, például Socket kapcsolaton vagy CORBA operáción keresztül érkezik a jel. Kevésbé hajlamos race condition-re, ezért általában ezt érdemes választani.
- **Out-of-band**: aszinkron esemény váltja ki, például UNIX SIGHUP szignál. Ez megszakítja a futó folyamatot.

---

## Ismert felhasználások

**Windows Service Control Manager (SCM)**: a Windows szolgáltatásokat (Windows Update, tűzfal, nyomtatási sor) az SCM kezeli. Minden szolgáltatás reagál a PAUSE, RESUME és TERMINATE üzenetekre. Ez a Component Configurator mintájának közvetlen megvalósítása.

**Java appletek**: a böngésző dinamikusan tölt le és indít el Java appleteket. A `java.applet.Applet` osztály `init()`, `start()`, `stop()` és `destroy()` metódusai pontosan a Component Configurator életciklus-interfészét valósítják meg.
...
**Egységes komponenshozzáférés**: minden komponens ugyanazt az interfészt implementálja. Az adminisztrátor egyforma módon kezeli az összes komponenst, függetlenül attól, hogy az adott komponens mit csinál. Ez a "principle of least surprise" elvét érvényesíti.

**Központi adminisztrálhatóság**: az inicializálás, szüneteltetés és leállítás egyetlen helyen, a Configurator-ban van kezelve. Nem kell minden komponens kódjába beleírni az adminisztrációs logikát.

**Dinamikus komponenscsatlakozás és vezérlés**: egy komponens újrakonfigurálható a meglévő kód módosítása, újrafordítása vagy a többi komponens újraindítása nélkül.

**Modularitás és optimalizálás**: a komponensek önálló DLL-ekbe vannak csomagolva, így külön fejleszthetők és tesztelhetők. A rendszer alkalmazkodhat a változó terheléshez.

**Hátrányok részletesen:**

**Biztonság csökkenése**: egy DLL-ből betöltött komponens megszemélyesíthet egy legitim komponenst. Egy hibás komponens megrongálhatja a megosztott állapotot.

**Teljesítmény overhead**: a Configurator inicializálja a komponenseket és linkeli őket a Repository-ba, ami plusz indirectiont jelent. Dinamikus linkelés esetén a fordítók extra szintű indirectiont adnak hozzá a metódushívásokhoz.

**Bonyolultság növekedése**: a Configurator, a Repository és a DLL-betöltési mechanizmus mind plusz kódrétegek, amelyek nehezíthetik a hibakeresést.

**Túl szűk közös interfész**: egyes komponensek inicializálása olyan összetett, hogy nem fér bele az egységes `init()` és `fini()` interfészbe.

---

## Kapcsolata más patternekkel

A Component Configurator belül **Wrapper Facade**-eket használ a DLL-betöltő OS API-k eléréséhez. A `dlopen()`, `dlsym()` és `dlclose()` UNIX hívásokat, illetve a Windows `LoadLibrary()`, `GetProcAddr()` és `CloseHandle()` hívásokat egy `DLL` Wrapper Facade osztály rejti el.

A **Reactor** patternnel együtt használják: a komponensek regisztrálhatják magukat egy Reactor-ban, amely eseményeket küld nekik. A Component Configurator felelős a komponensek betöltéséért, a Reactor a futásukért.
